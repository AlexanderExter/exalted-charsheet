              ...
              <div className="space-y-2">
                ...
                <div className="relative w-full h-3 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(100, ((character?.essence?.anima || 0) / 10) * 100)}%` }}
                  />
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {Object.entries(character.attributes || {}).map(([key, attr]: [string, any]) => {
                    const colorClass =
                      key === "fortitude"
                        ? "text-green-600"
                        : key === "finesse"
          ...
          <CardContent>
            <div className="max-h-96 overflow-y-auto">
              <table className="w-full">
                ...
                <tbody>
                  {Object.entries(character.abilities || {}).map(([key, ability]: [string, any]) => (
                    <tr key={key} className="border-b border-gray-200">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant={character?.dicePool?.attribute === "fortitude" ? "default" : "outline"}
                      size="sm"
                      onClick={() =>
                        updateCharacter({
                    ...
                    >
                      ...
                      <div className="text-xs opacity-75">
                        (
                        {calculateStatTotal(character?.attributes?.fortitude || { base: 0, added: 0, bonus: 0 })}
                        )
                      </div>
                    </Button>
                    <Button
                      variant={character?.dicePool?.attribute === "finesse" ? "default" : "outline"}
                      size="sm"
                      onClick={() =>
                        updateCharacter({
                    ...
                    >
                      ...
                      <div className="text-xs opacity-75">
                        (
                        {calculateStatTotal(character?.attributes?.finesse || { base: 0, added: 0, bonus: 0 })}
                        )
                      </div>
                    </Button>
                    <Button
                    ...
                    >
                      <div>Force</div>
                      <div className="text-xs opacity-75">
                        ({calculateStatTotal(character?.attributes?.force || { base: 0, added: 0, bonus: 0 })})
                      </div>
                    </Button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateCharacter({
                        dicePool: { ...character.dicePool, ability: value as keyof typeof character.abilities },
                      })
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {Object.keys(character?.abilities || {}).map((ability) => (
                        <SelectItem key={ability} value={ability}>
                          ...
                          {calculateStatTotal(character?.abilities?.[ability] || { base: 0, added: 0, bonus: 0 })})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
