...
export const SideCharacterEditor: React.FC<SideCharacterEditorProps> = ({
  ...
  const [newPoolType, setNewPoolType] = useState<DicePoolType>("primary");
  const [newPoolValue, setNewPoolValue] = useState(5);
  const [newPoolAction, setNewPoolAction] = useState("");

  // Add a new pool
  const addPool = () => {
    if (!newPoolAction.trim()) return;

    // Check for duplicate action
    if (sideCharacter.pools.some(p => p.action.toLowerCase() === newPoolAction.trim().toLowerCase())) {
      alert("A pool with this action already exists!");
      return;
    }
                ...
                <Input
                  ...
                  onChange={e =>
                    onUpdate({
                      battlegroup: {
                        ...sideCharacter.battlegroup!,
                        drill: Math.max(0, parseInt(e.target.value) || 0),
                      },
                    })
                  }
                  min={0}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="border-b pb-4">
            <Label className="mb-2 block">Add New Pool</Label>
            <div className="grid grid-cols-4 gap-3">
              <Select value={newPoolType} onValueChange={(v: DicePoolType) => setNewPoolType(v)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Input
                type="number"
                value={newPoolValue}
                onChange={e => setNewPoolValue(parseInt(e.target.value) || 0)}
                placeholder="Dice"
                min={0}
              />
              <Input
                value={newPoolAction}
                onChange={e => setNewPoolAction(e.target.value)}
                placeholder="Action (e.g., fighting)"
                className="col-span-1"
              />
              <Button onClick={addPool} disabled={!newPoolAction.trim()}>
          ...
          <div className="space-y-3">
            {sideCharacter.pools.map(pool => (
              <div key={pool.id} className="flex items-center gap-3 p-3 border rounded">
                <Select
                  value={pool.type}
                ...
                <Input
                  ...
                  className="flex-1"
                />
                <Button variant="destructive" size="sm" onClick={() => deletePool(pool.id)}>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {sideCharacter.qualities.map(quality => (
            <div key={quality.id} className="border rounded p-4 space-y-3">
          ...
          {sideCharacter.qualities.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-4">
              No qualities yet. Add one above.
            </p>
          )}
        </CardContent>
      </Card>
